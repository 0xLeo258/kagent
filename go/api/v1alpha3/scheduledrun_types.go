...
const (
	...
	// ScheduledRunTargetKindAgentTemplate is the AgentTemplate target kind.
	ScheduledRunTargetKindAgentTemplate = "AgentTemplate"
)
...
//   - InProgress: the execution is queued, creating its instance, dispatching, or running.
...
// +kubebuilder:validation:XValidation:rule="has(self.targetRef.kind) && self.targetRef.kind == 'AgentTemplate'",message="targetRef.kind must be AgentTemplate"
...
// +kubebuilder:validation:XValidation:rule="self.harnessRef == oldSelf.harnessRef",message="harnessRef is immutable"
// +kubebuilder:validation:XValidation:rule="has(self.harnessRef.name) && self.harnessRef.name.size() > 0 && self.harnessRef.name.size() <= 253 && self.harnessRef.name.matches('^([a-z0-9]([-a-z0-9]*[a-z0-9])?)([.][a-z0-9]([-a-z0-9]*[a-z0-9])?)*$')",message="harnessRef.name must be a valid DNS subdomain"
type ScheduledRunSpec struct {
	...
	// AgentTemplate resources. TargetRef is immutable after creation so
	// execution recovery always uses the original target.
	// +required
	TargetRef corev1.TypedLocalObjectReference `json:"targetRef"`

	// HarnessRef selects the same-namespace Harness used for each AgentInstance.
	// +required
...
type ScheduledRunExecution struct {
	// ID uniquely identifies this execution independently of its instance or task.
	// +required
	// +kubebuilder:validation:MinLength=1
	...
	// AgentInstanceID identifies the conversation created for this execution.
	// +optional
	// +kubebuilder:validation:MinLength=1
...
type ScheduledRunStatus struct {
	...
	// RecentExecutions contains the 100 most recent in-progress executions and
	// completed executions selected by spec.recentExecutionsLimit. Messages are
	// abbreviated to 1024 characters; complete records remain in execution history.
...
// +kubebuilder:resource:path=scheduledruns,singular=scheduledrun,categories=kagent
