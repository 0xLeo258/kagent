...
import { scheduledRunForInstance, scheduledRunMayReply, SCHEDULED_CONVERSATIONS } from "@/mocks/scheduledRuns";
...
export class MockChatClient implements ChatClient {
  ...
  private async *stream(input: SendMessageInput): AsyncIterable<ChatEvent> {
    ...
    if (instance?.scheduledRun && !scheduledRunMayReply(scheduledRunForInstance(instance.id))) {
      yield {
        type: "error",
...
const SEEDED_TRANSCRIPTS: Record<string, () => ChatMessage[]> = {
  ...Object.fromEntries(SCHEDULED_CONVERSATIONS.map(({ id }) => [id, () => [
